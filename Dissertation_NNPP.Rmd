...
processing_tap = raw_time %>%
  select(Participant, Tap1, Tap2, Tap3, Tap4) %>%
  mutate_if(is.character, tolower) %>%
  mutate_if(is.character, gsub, pattern = " ", replace = "") %>%
  pivot_longer(cols = c("Tap1", "Tap2", "Tap3", "Tap4"), names_to = "trial", values_to = "tap") %>%
  mutate(trial = dplyr::recode(trial, "Tap1" = "1", "Tap2" = "2", "Tap3" = "3", "Tap4" = "4"))
...
tableOne <- CreateTableOne(vars = c("word_count","utterance_count", "performance_ratio", "time", "word_min", "utterance_min", "TakeItSerious_Game_1", "TakeItSerious_Survey_1", "PS_Lkrt_rati", "PS_Lkt_conf", "PS_pcg_rati", "PS_talk_pcg_conf", "comfortable_PS", "Content_naming", "Content_prasing","Content_criticize", "Gender")
                                                                                                 , 
                           strata = c("Gender"), 
                           data = recoded_full_df %>% filter(Gender != "Non-binary" & Gender != "Prefer not to say" ) %>% select(word_count,utterance_count, performance_ratio, time, word_min, utterance_min, TakeItSerious_Game_1, TakeItSerious_Survey_1, PS_Lkrt_rati, PS_Lkt_conf, PS_pcg_rati, PS_talk_pcg_conf, comfortable_PS, Content_naming, Content_prasing,Content_criticize, Gender  ) %>%
                             data.frame())

df_baseline_comparison = recoded_full_df %>% select(Participant, baseline_ratio, Gender) %>%
  distinct()
  #                                   Stratified by Gender
  #                                    Man           Women          p      test
  # n                                     62           168                     
  # word_count (mean (SD))             65.32 (49.43) 58.08 (40.90)   0.264     
  # utterance_count (mean (SD))        31.27 (10.82) 28.16 (12.18)   0.078     
  # performance_ratio (mean (SD))       1.31 (0.20)   1.31 (0.19)    0.904     
  # time (mean (SD))                   64.06 (19.13) 70.96 (29.07)   0.085     
  # word_min (mean (SD))               58.57 (31.50) 52.45 (36.39)   0.245     
  # utterance_min (mean (SD))          30.71 (11.32) 26.00 (11.06)   0.005     
  # TakeItSerious_Game_1 (mean (SD))    3.84 (0.37)   3.80 (0.46)    0.527     
  # TakeItSerious_Survey_1 (mean (SD))  3.97 (0.18)   3.96 (0.19)    0.900     
  # PS_Lkrt_rati (mean (SD))            5.48 (1.30)   5.32 (1.35)    0.399     
  # PS_Lkt_conf (mean (SD))             6.32 (0.94)   6.15 (1.03)    0.246     
  # PS_pcg_rati (mean (SD))            74.55 (21.78) 75.08 (22.38)   0.873     
  # PS_talk_pcg_conf (mean (SD))        6.06 (1.04)   6.03 (0.94)    0.810     
  # comfortable_PS (mean (SD))          5.37 (1.72)   4.74 (1.76)    0.015     
  # Content_naming (mean (SD))          6.35 (1.29)   6.13 (1.36)    0.255     
  # Content_prasing (mean (SD))         1.84 (1.21)   2.54 (1.82)    0.029     
  # Content_criticize (mean (SD))       1.95 (1.49)   2.02 (1.24)    0.774     
  # Gender = Women (%)                     0 (0.0)     168 (100.0)  <0.001 

t.test(df_baseline_comparison$baseline_ratio[df_baseline_comparison$Gender == "Women"], df_baseline_comparison$baseline_ratio[df_baseline_comparison$Gender == "Man"])
# 	Welch Two Sample t-test
# 
# data:  df_baseline_comparison$baseline_ratio[df_baseline_comparison$Gender == "Women"] and df_baseline_comparison$baseline_ratio[df_baseline_comparison$Gender == "Man"]
# t = -2.1699, df = 53.847, p-value = 0.03444
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#  -0.134136266 -0.005298791
# sample estimates:
# mean of x mean of y 
#  1.283123  1.352840 

sd(df_baseline_comparison$baseline_ratio[df_baseline_comparison$Gender == "Women"], na.rm = T)
sd(df_baseline_comparison$baseline_ratio[df_baseline_comparison$Gender == "Man"])
...
scaled_df = all_content_full_df %>%
  select(word_count:TakeItSerious_Survey_1,
         SSS_self:Content_criticize,
         all_content:irrelevant_rate) %>% 
  scale() %>%
  data.frame()
...
unpopular_yes = recoded_full_df %>% mutate(reaction_yes = ifelse(reaction > 0, "1", "0"), 
  ...
  select(reaction_yes : ambiguous_yes)

scaled_df = cbind(scaled_df,unpopular_yes)


scaled_full_df = cbind(recoded_full_df %>%
  select(Participant, trial, Age:`TRANSFER STUDENT`, language), scaled_df) 
...
# random slope attempt and the error message
#  full_model_utterance_min_ratio = lmer(data = scaled_full_df, performance_ratio ~ baseline_ratio * utterance_min + (1 + utterance_min|Participant))
# Error: number of observations (=230) <= number of random effects (=234) for term (1 + utterance_min | Participant); the random-effects parameters and the residual variance (or scale parameter) are probably unidentifiable
...
# checking for R2
full_model_word_ratio = lmer(data = scaled_full_df, performance_ratio ~ baseline_ratio * word_count + (1|Participant))
full_model_utterance_ratio = lmer(data = scaled_full_df, performance_ratio ~ baseline_ratio * utterance_count + (1|Participant))

full_model_utterance_rate_ratio = lmer(data = scaled_full_df, performance_ratio ~ baseline_ratio * utterance_min + (1|Participant))

full_model_word_rate_ratio = lmer(data = scaled_full_df, performance_ratio ~ baseline_ratio * word_min + (1|Participant))

tab_model(full_model_utterance_ratio, 
          full_model_word_ratio, 
          full_model_utterance_rate_ratio, 
          full_model_word_rate_ratio)
```
...
```{r tap versus actual performance, include=FALSE}
names(processing_performance)[names(processing_performance) == "L"] = "Participant"
processing_performance$trial = as.character(processing_performance$trial)

df_tap_performance = right_join(processing_tap, processing_performance, by= c("Participant", "trial")) 
...
original_col_list = c("tap", "actual_turn", "performance_ratio", "turn_assume_perfect") 
renamed_col_list= c("raw tap", "calculated turn", "ratio", "calcualted perfect")


correlation_zeroOrder_plot(df_tap_performance, original_col_list,renamed_col_list)

cor.test(df_tap_performance$tap, df_tap_performance$actual_turn)

cor.test(df_tap_performance$tap, df_tap_performance$performance_ratio)

getwd()

```

```{r distribution of PS usage to see if ppl followed rules, include=FALSE}
# processing_word_utterance_long is the dataset 
ggplot(data = processing_word_utterance_long, aes(x = word_count)) +
  geom_histogram()
```
